...
for train, test in kfold.split(stores, origin_y):
    model10_combine = ResNet50(input_shape=stores.shape[1:])

    model10_combine.compile(optimizer='adam', loss='mse', metrics=['mse'])
    for _ in range(10):
        lines10_combine = model10_combine.fit(stores[train], y_data[train], epochs = 5, batch_size = 16, validation_split=0.2)
        preds10_combine_ = model10_combine.evaluate(stores[test], y_data[test])
        print ("model10_combine Loss = " + str(preds10_combine_[0]))
        print ("model10_combine Test Accuracy = " + str(preds10_combine_[1]))
        
    preds10_combine = model10_combine.evaluate(stores[test], y_data[test])
